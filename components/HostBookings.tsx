...
const HostBookings: React.FC = () => {
  ...
  const BookingCard: React.FC<{ booking: Booking; showActions?: boolean }> = ({ booking, showActions = false }) => {
    const [localResponseMessage, setLocalResponseMessage] = useState('');
        ...
        <div className="md:w-2/3 p-6">
          ...
          {showActions && booking.status === BookingStatus.PENDING && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              ...
              <textarea
                value={localResponseMessage}
                onChange={(e) => setLocalResponseMessage(e.target.value)}
                placeholder="Añade un mensaje personalizado para el cliente..."
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 text-gray-900 placeholder-gray-500"
                rows={3}
                style={{ fontSize: '14px', lineHeight: '1.5' }}
              />
              {localResponseMessage && (
                <div className="mt-2 p-2 bg-white border border-gray-200 rounded text-sm text-gray-700">
                  <strong>Vista previa:</strong> {localResponseMessage}
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {showActions && booking.status === BookingStatus.PENDING && (
              <>
                <button
                  onClick={() => handleBookingAction(booking.id, BookingStatus.CONFIRMED, localResponseMessage)}
                ...
                <button
                  onClick={() => handleBookingAction(booking.id, BookingStatus.REJECTED, localResponseMessage)}
                  disabled={processingAction === booking.id}
